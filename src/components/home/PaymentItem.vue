...
export default {
    ...
    created(){
        ...
        this.data = this.$route.query.shangPing
        // console.log(this.$route.query.shangPing)
    },
    methods:{
        history() {
            history.go(-1)
        ...
        qwe(){
            if($('#r2').is(':checked')){
                ...
                var orderId = this.orderIds()[0]
                var Money = this.orderIds()[1]
                this.wxPay(orderId,Money)
            }
        },
        onInput(key) {
        ...
        orderIds(){
            ...
            for (var i = 0; i < this.data.length; i++) {
                ...
                Money += this.data[i].data.orderMoney
            }
            var orderId = id.join(',')
        ...
        yue(orderId,Money){
            ...
            this.$axios.post(this.$httpUrl.pay,$.param({access_type:'WXH5', wxh: this.$storage.getStore('wx'), openId:this.$storage.getStore('openIds'),orderIds:orderId,payType:3,payPwd:this.value}) )
            .then(response => {
                this.show = false
                ...
                if(response.data.code == 200){
                    ...
                    setTimeout(() => {
                        this.$vux.toast.hide()
                        this.$router.push({path:'Complete',query:{orderIds:orderId,Moneys:Money}})
                    }, 1000)
                }else{
        ...
        wxPay(orderId,Money){
            if (typeof WeixinJSBridge == "undefined"){
                ...
                this.onBridgeReady(orderId,Money);
            }
        },
        onBridgeReady(orderId,Money){
            // console.log(orderId)
            this.$axios.post(this.$httpUrl.h5pay,$.param({access_type:'WXH5',orderIds:orderId,payType:2,
                wxh:this.$storage.getStore('wx'),openId:this.$storage.getStore('openIds')
            }) )
            .then(response => {
                // console.log(response.data)
                if(response.data.code == 200){
                    this.wxzhifu(response.data.data,orderId,Money)
                }else{
                    this.$vux.loading.show({
                        text: response.data.message
                    })
            ...
            WeixinJSBridge.invoke(
                ...
                (res) => {
                    if (res.err_msg == "get_brand_wcpay_request:ok") {// 使用以上方式判断前端返回,微信团队郑重提示：res.err_msg将在用户支付成功后返回    ok，但并不保证它绝对可靠。 
                        this.$router.replace({path:'Complete',query:{orderIds:orderId,Moneys:Money}})
                    } else {
